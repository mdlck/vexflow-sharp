...
Current status: 537 NUnit tests passing, approximately 25,500 lines of C#.
...
|---------|--------|---------|
| `VexFlowSharp.Common` | `netstandard2.1` | Core library — all notation classes, formatting engine (`Formatter`, `TickContext`, `ModifierContext`), high-level API (`Factory`, `EasyScore`, `System`), and rendering abstractions. No platform-specific dependencies. |
| `VexFlowSharp.Tests` | `net10.0` | NUnit test suite. Includes a SkiaSharp-based `SkiaRenderContext` for pixel-comparison testing against VexFlow JS reference images. |
| `VexFlowSharp.Unity` | Unity UPM package | Provides `VexFlowElement` (a custom `VisualElement`) and `UIElementsRenderContext` for rendering via Unity UI Toolkit / Painter2D. Requires Unity 2022.1+ (`Painter2D.BezierCurveTo` was introduced in Unity 2022.1). |
...
The following example renders a treble staff with four quarter notes using EasyScore. The `SkiaRenderContext` is defined in `VexFlowSharp.Tests`.
...
Reference `VexFlowSharp.Common` as a project reference or NuGet package. Add a rendering backend — the `SkiaRenderContext` in `VexFlowSharp.Tests` can be extracted and adapted for standalone use.
